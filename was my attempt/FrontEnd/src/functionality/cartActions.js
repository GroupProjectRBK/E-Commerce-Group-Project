...
axios.get(`/api/cart/${id}`).then(res => dispatch({
            type: GET_CART,
            payload: res.data
...
axios.post(`/api/cart/${id}`, {productId, quantity}).then(res => dispatch({
            type: ADD_TO_CART,
            payload: res.data
})).catch(err => dispatch(returnErrors(err.response.data, err.response.status)));
}
...
axios.delete(`/api/cart/${userId}/${itemId}`).then(res => dispatch({
